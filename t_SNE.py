...
def procesar_tsne(df):
    ...
    # Verificar si la columna 'Internet Service' está presente
    if 'Internet Service' in df_limpio.columns:
        ...
        x = np.asanyarray(df_limpio.drop(columns=['Internet Service']))[:n_sample, :]  # Cambiado a 'Internet Service'
        y = np.asanyarray(df_limpio['Internet Service'])[:n_sample].ravel()  # Cambiado a 'Internet Service'
